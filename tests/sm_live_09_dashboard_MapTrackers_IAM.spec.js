...
test('View Map Trackers and Validate API Response', async ({ page }) => {
  ...
  const allUnitStatusSelectors = [
    page.locator('.accessPlatform[data-type="inAggrMode"]').first(),
    page.locator('.accessPlatform[data-type="inAggrMode"]'),
    page.locator('a.accessPlatform[data-type="inAggrMode"]'),
  ];
  ...
  for (const selector of allUnitStatusSelectors) {
    try {
      ...
      console.log('Clicked In Aggressive Mode');
  ...
  if (!locationHistoryClicked) {
    throw new Error('Failed to click In Aggressive Mode');
  }

  console.log('Waiting for page to load after clicking In Aggressive Mode...');
  ...
  // Check if it's the expected In Aggressive Mode URL
  if (newPageUrl.includes('driverId=agg') || newPageUrl.includes('index2.php')) {
    console.log('Successfully opened In Aggressive Mode in new tab with correct URL');
  } else {
    console.log('Warning: URL pattern is correct but may not be the expected In Aggressive Mode page');
  }
  
  console.log('Successfully navigated to In Aggressive Mode page in new tab');
